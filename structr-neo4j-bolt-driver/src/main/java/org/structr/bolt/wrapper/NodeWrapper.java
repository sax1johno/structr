...
public class NodeWrapper extends EntityWrapper<org.neo4j.driver.v1.types.Node> implements Node {
	...
	public Iterable<Relationship> getRelationships() {
		...
		if (list == null) {
			...
			list = Iterables.toList(Iterables.map(mapper, tx.getRelationships("MATCH (n)-[r]-() WHERE ID(n) = {id} RETURN r", map, -1, false)));
	...
	public Iterable<Relationship> getRelationships(final Direction direction, final RelationshipType relationshipType) {
		...
		if (list == null) {
			...
			switch (direction) {
				...
				case BOTH:
					list = Iterables.toList(Iterables.map(mapper, tx.getRelationships("MATCH (n)-[r:" + relationshipType.name() + "]-() WHERE ID(n) = {id} RETURN r", map, -1, false)));
					break;
